...
fof(out2ax1, axiom, (![X,T]: ((train(T) & go(X,T) & ( (switch(X,s1) = out2 & at(X,T,s1)) | (switch(X,s2) = out2 & at(X,T,s2)) )) <=> at(succ(X),T,out2)))).
fof(out1ax1, axiom, (![X,T]: ((train(T) & go(X,T) & ( (switch(X,s1) = out1 & at(X,T,s1)) | (switch(X,s2) = out1 & at(X,T,s2)) )) <=> at(succ(X),T,out1)))).
...
fof(nodesExclusivity, axiom, ((in2 != in1) & (in2 != s2) & (in2 != s1) & (in2 != out2) & (in2 != out1) & (in1 != s2) & (in1 != s1) & (in1 != out2) & (in1 != out1) & (s2 != s1) & (s2 != out2) & (s2 != out1) & (s1 != out2) & (s1 != out1) & (out2 != out1))).
